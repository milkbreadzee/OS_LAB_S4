...
void main()
{
    ...
    difference,temp,before[20],after[20],temp1=0,temp2=0;
    ...
    for(i=1;i<=n;i++)   // Note that i varies from 1 to n instead of 0 to n-1
    {
        ...
        //then pushing that to array before
        if(temp>headposition)
        {
            before[temp1]=temp; //temp1 is the index variable of before[]
            temp1++; //incrementing temp1
        }
        else    //else if temp < current headposition,then push to array after[]
        {   
            after[temp2]=temp; //temp2 is the index variable of after[]
            temp2++;
        }
    }
    
    //Now we have to sort the two arrays
    //SORTING array before[] in ascending order
    for(i=0;i<temp1-1;i++)
    {
        for(j=i+1;j<temp1;j++)
        {
    ...
    //SORTING array after[] in descending order
    for(i=0;i<temp2-1;i++)
    {
        for(j=i+1;j<temp2;j++)
        {
    ...
    //Copying first array before[] into queue[]
    ...
    //Copying second array after[] after that first one is copied, into queue[]
